...
import '../screen/mysalon/barber_details_widget.dart';
...
class LocalNotificationService {
  ...
  static Future<void> initialize() async {
    ...
    const AndroidNotificationChannel channel = AndroidNotificationChannel(
      'dizisalonpartner', // same as manifest
...
static Future<void> _onNotificationTap(
  ...
  static Future<void> handleNotificationTap(String payload) async {
    ...
    final String appointmentId = data['appointment_id']?.toString() ?? "";
    if (appointmentId.isEmpty) {
      debugPrint("❌ appointment_id missing in payload");
      return;
    }

    // Call API
    final apiResponse = await controller.allOrderList(
      apponmentId: appointmentId,
    );
  ...
  MaterialPageRoute(
    builder: (_) => customerDetails(orderItem: appointment,fromNotification: true,),
  ),
